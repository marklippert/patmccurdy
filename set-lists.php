...
if ($_SERVER['QUERY_STRING'] != "" && !isset($_REQUEST['search'])) {
  $result = $mysqli->query("SELECT * FROM setlists WHERE id ='" . $_SERVER['QUERY_STRING'] . "'");
...
if ($_SERVER['QUERY_STRING'] != "" && !isset($_REQUEST['search'])) {
  ...
  if (isset($_REQUEST['search'])) {
    ...
    $result = $mysqli->query("SELECT * FROM setlists WHERE approved = 'on' AND set1 LIKE \"%" . $_REQUEST['search'] . "%\" OR set2 LIKE \"%" . $_REQUEST['search'] . "%\" OR set3 LIKE \"%" . $_REQUEST['search'] . "%\" ORDER BY date DESC");
    
    $message = ($result->num_rows > 0) ? "The following shows matched your search:" : "Sorry, nothing found for \"" . $_REQUEST['search'] . "\"";
    echo "<h2>$message</h2><br>\n";
  } else {
    // Index page
    // Get last time the setlists table was updated
    $result = $mysqli->query("SHOW TABLE STATUS LIKE 'setlists'");
    $row = $result->fetch_array(MYSQLI_ASSOC);
    $lastupdate = date("F j, Y", strtotime($row['Update_time']));
    
    // Now get the actual data
    $result = $mysqli->query("SELECT * FROM setlists WHERE approved = 'on' ORDER BY date DESC");
    ?>
    ...
    <div class="half-left centered">
      <strong><?php echo $result->num_rows; ?> shows currently available.</strong>
    </div>
