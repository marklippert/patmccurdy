...
<div id="admin-schedule" class="flex main-cols">
  <div>
    ...
    if (!empty($_POST['searchsubmit'])) {
      ...
      $searchresult = $mysqli->query("SELECT * FROM schedule WHERE event LIKE '%" . $_POST['search'] . "%' OR venue LIKE '%" . $_POST['search'] . "%' ORDER BY date ASC");
      
      if ($searchresult->num_rows > 0) {
        echo "<table>\n";

        while($search = $searchresult->fetch_array(MYSQLI_ASSOC)) {
          ?>
          <tr class="show">
            <td class="controls">
...
<div>
  ...
  <table>
    ...
    $shows = $mysqli->query("SELECT * FROM schedule WHERE date >= '$first_day' AND date <= '$last_day' ORDER BY date ASC");
    
    while($show = $shows->fetch_array(MYSQLI_ASSOC)) {
      ?>
      <tr><td colspan="3">&nbsp;</td></tr>
      <tr class="show">
        <td class="controls">
        ...
        <td class="show-info">
          ...
          if ($show['venue'] != "") {
            ...
            if ($show['date'] > strtotime(date("n/j/Y", $show['date']))) echo "<br>\n" . date("g:ia", $show['date']);
